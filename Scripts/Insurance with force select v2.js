var initForceInsurance = setInterval(function () {
	...
	function clickContinue() {
		...
		if (continueButton) {
			continueButton.addEventListener('click', function() {
				var botonNoQuieroSeguro = document.querySelector('#container-no-seguro');
				var buttonAdd = document.querySelector('.rounded-primary-btn.white-blue');
				verificarRut();
				if (botonNoQuieroSeguro) {
					console.log("A");
					addInvalidClass();
					addAlert();
				} else if (!botonNoQuieroSeguro && buttonAdd) {
					console.log("B");
					removeAlert2();
					removeInvalidClass2();
					cerrarModal();
	...
	if (culture === 'es-CL' || culture === 'es-PE') {
		addCSS();
		addButton();
		clickContinue();
