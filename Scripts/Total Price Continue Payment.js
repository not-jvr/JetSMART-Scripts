var initTotalPriceContinuePayment = setInterval(function () {
	if (typeof bookingData === "undefined" || window.location.pathname.toLowerCase() !== '/v2/payment') return;
	clearInterval(initTotalPriceContinuePayment);
	...
	function addPriceMSG() {
		var container = document.querySelector('.payment-footer-grid');
		var precio = price();
		var total = 'Total ';
		if (postBooking) {
			var tasas = '';
		...
		var mensaje = '<div class="price-continue-payment">'+ total + ' <strong>' + precio + '</strong> ' + tasas +'</div>';

		if (!document.querySelector('.price-continue-payment') && container) {
