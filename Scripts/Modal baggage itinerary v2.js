...
function countDays() {
  var outboundFlight = new Date(bookingData.OutboundJourney.DepartureDate);
  var returnFlight = new Date(bookingData.ReturnJourney.DepartureDate);

  var timeDifference = returnFlight.getTime() - outboundFlight.getTime();
  var daysDifference = Math.floor(timeDifference / (1000 * 3600 * 24));

  return daysDifference >= 3;
}

function hoursForOutbound() {
    var outboundFlight = new Date(bookingData.OutboundJourney.DepartureDate);
    var currentTime = new Date();
    var timeDifference = outboundFlight.getTime() - currentTime.getTime();
...
function verificacion() {
    if (!haveBaggage() && esIda() && checkinON() && isCulture() && countDays() && hoursForOutbound() && bookingData.ReturnJourney !== null) {
        return true;
    } else {
        return false;
    }
