var initFullPriorityv3 = setInterval(function() {
	if (typeof bookingData === "undefined" || window.location.pathname !== '/V2/Extras') return;
	clearInterval(initFullPriorityv3);
	...
	function changeText() {
		...
		switch (culture) {
		case 'en-US':
			subtitulo = 'Forget about long lines and be our first priority!';
			break;
		case 'pt-BR':
			subtitulo = 'Esqueça as longas filas e seja nossa primeira prioridade!';
			break;
		default:
			subtitulo = '¡Olvidate de las largas filas y sé nuestra primera prioridad!';
		}
	...
	function insertImage() {
		...
		var linkimg;
		if (!document.querySelector('.priority-boarding-img')) {
			switch (culture) {
			case 'en-US':
	...
	function addTitle() {
		var targetElement = document.querySelector('[data-test-id="extras-priority-boarding-container"] .col-xs-1.col-sm-1-2.col-sm-offset-1-4');
	...
	function addAfter() {
		var boardingButton = document.querySelector('[data-test-id="extras-priority-boarding-container"] .btn-boarding.select-dependent-text');

		if (boardingButton) {
			if (!boardingButton.classList.contains('with-after')) {
				boardingButton.classList.add('with-after');
			} else {
				console.log('El elemento ya tiene el after pseudo-elemento.');
			}
		} else {
